...
Class is using the following global variables:
	...
	"public nvarchar SAPRetnCode" of type [nvarchar]
	"public nvarchar SAPRetnMsg" of type [nvarchar]
	"public nvarchar SAPKey" of type [nvarchar]
*/
namespace genesis;
	...
	*/
	public static function ini_RecID($recid) {	
		...
		try {
			...
			$conn->Open();
			$query = " SELECT RecID, [Status], [CO], [Branch], [TxTp], [StockCode], [LinkCode], [Description1], [PriceCode], [Account], [SubAccount], [DocNo], [Text1], [Text2], [Text3], [CreateDate], [CreateTime], [CreateUser], [CreateMacID], [ProcessDate], [ProcessTime], [ProcessUser], [ProcessMacID], [ReturnMsg], [SAPRetnCode], [SAPRetnMsg], [SAPKey] FROM [SVRTrig]  WHERE [RecID]=:recid  ";
			...
			while($records  = $stmt->fetch( PDO::FETCH_ASSOC )) {
				...
				$instance->sapretncode = $records["SAPRetnCode"];
				$instance->sapretnmsg = $records["SAPRetnMsg"];
				$instance->sapkey = $records["SAPKey"];
			}
		} catch (Exception $ex) {
	...
	*/
	public static function ini_StockCode($stockcode) {	
		...
		try {
			$conn = new MyConnection();
			$conn->Open();
			$query = " SELECT RecID, [Status], [CO], [Branch], [TxTp], [StockCode], [LinkCode], [Description1], [PriceCode], [Account], [SubAccount], [DocNo], [Text1], [Text2], [Text3], [CreateDate], [CreateTime], [CreateUser], [CreateMacID], [ProcessDate], [ProcessTime], [ProcessUser], [ProcessMacID], [ReturnMsg], [SAPRetnCode], [SAPRetnMsg], [SAPKey] FROM [SVRTrig]  WHERE [StockCode]=:stockcode  ";
			$stmt = $conn->getStatement($query);
